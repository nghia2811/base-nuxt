<template>
    <div class="header-logo" @click="() => $router.push('/')">
        <div class="d-flex">
            <div class="logo-img" :style="{'color': !!color ? color: '#fff'}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 122.88 111.48">
            ...
            <span class="logo-title title-font" :style="{'color': !!color ? color: '#fff'}">{{ title }}</span>
        </div>
    </div>
</template>
...
<style lang="scss" scoped>
    .header-logo {cursor: pointer; 
        .logo-img { margin-right: 4px;
            > * { height: 30px;width: auto; display: block;margin: 0 10px; }
            svg {fill: currentColor; }
        }
        .logo-title {font-size: 30px; line-height: 1; font-weight: 800 }
    }
</style>
